...
export default class GiftWrapping {
  ...
  _bindModalEvents($modal) {
    ...
    $('[data-giftwrap-select]').change((event) => {
      ...
      $(`[data-giftwrap-image-${index}]`).addClass('is-hidden');
      $(`[data-giftwrap-image-${index}="${id}"]`).removeClass('is-hidden');

      if (allowMessage) {
        $(`[data-giftwrap-message-${index}]`).removeClass('is-hidden');
      } else {
        $(`[data-giftwrap-message-${index}]`).addClass('is-hidden');
      }
    });
